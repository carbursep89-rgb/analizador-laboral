...
function VistaUpload({ archivos, setArchivos, cargando, error, onAnalizar, progreso }) {
    ...
    <div className="fade-in">
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>
          Sube todos los PDFs del período. Se procesarán uno a uno automáticamente.
        </p>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.6 }}>
          Certificados Previred · Liquidaciones · Finiquitos · Cartas de término
        </div>
      </div>
        ...
        <div style={{ marginTop: 24 }}>
            ...
            {archivos.map((f, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontSize: 16 }}>
                  {progreso.actual > i ? "✅" : cargando && progreso.actual === i ? "⟳" : "📄"}
                </span>
            ...
            <div style={{ marginTop: 16, background: "#0f2744", borderRadius: 10, padding: 16, border: "1px solid #1e3a5f" }}>
              ...
              <div style={{ height: 6, background: "#0f172a", borderRadius: 3, overflow: "hidden" }}>
                <div style={{
                  ...
                  background: "#3b82f6", borderRadius: 3, transition: "width 0.4s ease"
                }} />
              </div>
            </div>
          )}
          ...
          {!cargando && (
            <div style={{ marginTop: 20, display: "flex", gap: 10 }}>
              <button className="btn" onClick={onAnalizar}
      ...
      <div style={{ marginTop: 32, background: "#1e293b", borderRadius: 12, padding: 20, border: "1px solid #334155" }}>
        ...
        {[
          ["📑", "Certificado Cumplimiento período anterior (N-1)"],
          ["📑", "Certificado Cumplimiento período actual (N)"],
          ["🏦", "Comprobantes Previred (AFP, Salud, Mutual, Caja, Seguro Social)"],
          ["💵", "Liquidaciones de sueldo de todos los trabajadores"],
          ["✍️", "Finiquitos firmados ante notario"],
      ...
      <div style={{ background: "#1e293b", borderRadius: 12, border: "1px solid #334155", overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table>
            <thead>
              <tr>
                {["#","RUT","Nombre","Cargo","Estado","Liquid.","AFP","Salud","Mutual","C.Comp","Seg.S","Finiquito","Obs."].map((h) => (
    ...
    <div className="fade-in">
      ...
      {!resultado.desvinculados?.length ? (
        <div style={{ textAlign: "center", paddingTop: 60 }}>
          ...
          <div style={{ color: "#4ade80", fontWeight: 700 }}>Sin desvinculados registrados</div>
        </div>
      ) : (
        <div style={{ background: "#1e293b", borderRadius: 12, border: "1px solid #334155", overflow: "hidden" }}>
          <table>
...
export default function App() {
  ...
  const analizar = async () => {
    ...
    try {
      ...
      // FASE 1: procesar cada archivo individualmente
      for (let i = 0; i < archivos.length; i++) {
        ...
        const res = await fetch("/api/analizar", {
          ...
          body: JSON.stringify({ archivo: base64, nombre: f.name, fase: "extraer" }),
        });
      ...
      // FASE 2: consolidar todo en JSON final
      setProgreso({ actual: archivos.length, total: archivos.length + 1, mensaje: "Generando resumen final..." });

      const resFinal = await fetch("/api/analizar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fase: "consolidar", contexto: contextoAcumulado }),
      });

      const dataFinal = await resFinal.json();
      if (dataFinal.error) throw new Error(dataFinal.error);

      setResultado(dataFinal.resultado);
      setHistorial((prev) => [{
        id: Date.now(),
        fecha: new Date().toLocaleDateString("es-CL"),
        obra: dataFinal.resultado.empresa?.obra || "Sin nombre",
        periodo: dataFinal.resultado.empresa?.periodo || "",
        empresa: dataFinal.resultado.empresa?.razon_social || "",
        trabajadores: dataFinal.resultado.resumen?.total_trabajadores || 0,
        data: dataFinal.resultado,
      }, ...prev.slice(0, 9)]);

      setProgreso({ actual: archivos.length + 1, total: archivos.length + 1, mensaje: "¡Análisis completado!" });
      setVista("dashboard");
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <main style={{ marginLeft: 220, flex: 1, padding: "36px 36px 48px", maxWidth: "calc(100vw - 220px)", minHeight: "100vh" }}>
        {vista === "upload"        && <VistaUpload archivos={archivos} setArchivos={setArchivos} cargando={cargando} error={error} onAnalizar={analizar} progreso={progreso} />}
