...
const PROMPT_SISTEMA = `Eres un experto en gestion de recursos humanos y cumplimiento laboral en Chile, especializado en contratos de obra y subcontratacion bajo la Ley 20.123.
...
- Estado de cotizaciones (AFP, salud, mutual, caja compensacion, seguro social)
- Periodos, montos, instituciones
- Finiquitos, causales de termino, fechas
- Cualquier dato laboral relevante
Se exhaustivo. Responde solo con el texto extraido, sin JSON.`;

const PROMPT_FINAL = `Eres un experto en cumplimiento laboral en Chile (Ley 20.123).
Con base en los datos extraidos de los documentos, genera un JSON con este formato EXACTO (sin texto adicional, solo JSON):

{
  ...
  "trabajadores": [{
    ...
    "cargo": "",
    "estado": "Activo",
    "liquidacion": "no_verificado",
    "afp": { "institucion": "", "estado": "no_verificado" },
    "salud": { "institucion": "", "estado": "no_verificado" },
    "mutual": "no_verificado",
    "caja_comp": "no_verificado",
    "seg_social": "no_verificado",
    "finiquito": "N/A",
    "causal_termino": "",
    "observaciones": ""
  }],
  "desvinculados": [{
    ...
    "nombre": "",
    "ultima_nomina": "",
    "estado_finiquito": "pendiente",
    "tipo_documento": "",
    "causal": ""
  }],
  "alertas": [{ "nivel": "informativo", "mensaje": "" }]
}

REGLAS CRITICAS - SEGUIR ESTRICTAMENTE:
1. NUNCA marques algo como ok si no tienes el documento que lo acredita. Si no hay documento = no_verificado.
2. Si no se cargaron liquidaciones de sueldo: liquidacion = no_verificado para TODOS.
3. Si no se cargaron comprobantes Previred AFP: afp.estado = no_verificado para TODOS.
4. Si no se cargaron comprobantes Previred Salud: salud.estado = no_verificado para TODOS.
5. Si no se cargaron comprobantes Mutual: mutual = no_verificado para TODOS.
6. Si no se cargaron comprobantes Caja Compensacion: caja_comp = no_verificado para TODOS.
7. Si no se cargaron comprobantes Seguro Social: seg_social = no_verificado para TODOS.
8. COMPARACION DE NOMINAS: Si tienes datos de dos periodos distintos (ejemplo dic 2025 y ene 2026), compara la lista de RUTs. Los RUTs que aparecen en el periodo anterior pero NO en el periodo actual son DESVINCULADOS. Agregarlos en el array desvinculados con estado_finiquito pendiente si no hay finiquito.
9. Si no hay finiquito ni carta DT para un desvinculado: estado_finiquito = pendiente.
10. Agrega una alerta nivel informativo indicando exactamente que documentos fueron cargados y cuales faltan.
11. Responde SOLO con el JSON valido, sin texto antes ni despues, sin bloques markdown.`;
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>Sube los PDFs del periodo. Se procesaran uno a uno automaticamente.</p>
      </div>
      ...
      <div style={{ background: "#1e293b", borderRadius: 12, padding: 16, border: "1px solid #334155", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          />
          <button onClick={() => setShowKey(!showKey)} className="btn" style={{ background: "#334155", color: "#94a3b8", padding: "8px 12px" }}>
            {showKey ? "Ocultar" : "Ver"}
          </button>
        </div>
        <div style={{ fontSize: 10, color: "#475569", marginTop: 6 }}>
          La key se usa solo en tu navegador. Obtenla en console.anthropic.com
        </div>
      </div>
      ...
      >
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: "#e2e8f0", marginBottom: 8 }}>Arrastra los PDFs aqui o haz clic para seleccionar</div>
        <div style={{ fontSize: 13, color: "#475569" }}>Certificados Previred · Liquidaciones · Finiquitos · Cartas de termino</div>
      </div>
...
function VistaDashboard({ resultado, onExportar, onIrUpload }) {
  if (!resultado) return (
    <div className="fade-in" style={{ textAlign: "center", paddingTop: 100 }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 800, color: "#e2e8f0", marginBottom: 8 }}>Sin analisis activo</div>
      <div style={{ fontSize: 13, color: "#64748b", marginBottom: 24 }}>Carga los documentos del periodo para comenzar</div>
  ...
  return (
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "#f1f5f9" }}>{resultado.empresa?.obra || "Analisis Laboral"}</h1>
      ...
      {criticas.length > 0 && (
        <div style={{ background: "#450a0a", border: "1px solid #991b1b", borderRadius: 10, padding: "14px 16px", marginBottom: 20 }}>
          <div style={{ display: "flex", gap: 10 }}>
            <span>🔴</span>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#fca5a5", marginBottom: 6 }}>{criticas.length} alerta{criticas.length > 1 ? "s" : ""} critica{criticas.length > 1 ? "s" : ""}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 12, marginBottom: 28 }}>
        ...
        <KPI icon="💼" label="C.Compensacion" value={`${r.caja_comp_ok || 0}/${r.total_trabajadores || 0}`} bg="#052e16" color="#4ade80" />
      ...
      <div style={{ background: "#1e293b", borderRadius: 12, padding: 22, border: "1px solid #334155" }}>
        ...
        <BarraProgreso label="Caja de Compensacion"      ok={r.caja_comp_ok}  total={r.total_trabajadores} />
    ...
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: "#64748b", fontSize: 13, marginTop: 2 }}>{resultado.trabajadores?.length || 0} trabajadores en el periodo</p>
        </div>
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginTop: 2 }}>{resultado.desvinculados?.length || 0} trabajadores con termino de contrato</p>
      </div>
      {!resultado.desvinculados?.length ? (
        ...
        <div style={{ background: "#1e293b", borderRadius: 12, border: "1px solid #334155", overflow: "hidden" }}>
          <table>
            <thead><tr>{["#","RUT","Nombre","Ultima Nomina","Estado Finiquito","Tipo Documento","Causal"].map((h) => (
...
function VistaAlertas({ resultado }) {
  ...
  const grupos = [
    { items: criticas, label: "Critico",     color: "#fca5a5", bg: "#450a0a", border: "#7f1d1d", icon: "🔴" },
    { items: atencion, label: "Atencion",    color: "#fde68a", bg: "#451a03", border: "#78350f", icon: "🟡" },
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginTop: 2 }}>{criticas.length} criticas · {atencion.length} atencion · {info.length} informativas</p>
      </div>
  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, marginTop: 2 }}>{historial.length} analisis en esta sesion</p>
      </div>
...
export default function App() {
  ...
  const analizar = async () => {
    ...
    setProgreso({ actual: 0, total: archivos.length + 1, mensaje: "Iniciando analisis..." });
    ...
    try {
      ...
      for (let i = 0; i < archivos.length; i++) {
        ...
        if (i < archivos.length - 1) await new Promise(r => setTimeout(r, 12000));
      }
      ...
      const jsonTexto = await llamarClaude(PROMPT_FINAL, [{
        role: "user",
        content: `Datos extraidos de los documentos:\n${contextoAcumulado}\n\nGenera el JSON completo de analisis.`
      }], 8000);

      let clean = jsonTexto.replace(/```json|```/g, "").trim();
      const jsonMatch = clean.match(/\{[\s\S]*\}/);
      if (jsonMatch) clean = jsonMatch[0];
      clean = clean.replace(/[\u0000-\u001F\u007F-\u009F]/g, " ").replace(/,(\s*[}\]])/g, "$1");
